# ADR 0001: SQL Template 批次匯入匯出與預覽工作台多模式架構設計

- **狀態**：Accepted (已採用)
- **日期**：2026-08-28
- **決策者**：DataStudio 前端架構團隊
- **相關議題**：SQL Template 匯入/匯出功能、批次編輯情境與 SQL 工作台組件重用平衡

---

## 1. 背景與問題陳述 (Context & Problem Statement)

在 DataStudio 數據平台中，使用者需要能夠將全量 SQL Template 資產匯出為標準 JSON，以及批次匯入（上限 10 筆）進行預覽、檢閱、修改與儲存/送審。
...
3. **視覺沉穩與企業風格規範**：
   - 需避免使用非標準的 Emoji（如 ⚠️, 🚀, 💾），改採符合 Ant Design 企業級規範的扁平化狀態圓點與 SVG 圖示。
...
5. **企業風格去 Emoji 化**：
   - 全面替換為 Ant Design 風格的狀態圓點（`.status-dot-pending`, `.status-dot-saved`, `.status-dot-error`）與純量 SVG 圖示。
